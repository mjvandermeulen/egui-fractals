...
pub fn max_depth_with_branches(
    ...
    if replace_line {
        return max_painted_line_count.ilog(branch_count) as usize;
    }

    // copied from gemini
    let argument = (max_painted_line_count * (branch_count - 1) + 1) as f32;
    let max_depth_float = (argument.ln() / (branch_count as f32).ln()) - 1.0;
